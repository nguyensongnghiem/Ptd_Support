...
const Header = () => {
  ...
  const { auth, logout, isAuthenticated } = useAuth(); // Destructure isAuthenticated directly
  ...
  // Use useMemo to safely decode and get username, only re-calculating if auth.accessToken changes
  const username = useMemo(() => {
    if (auth?.accessToken) {
      try {
        ...
        return "Người dùng"; // Fallback if token is invalid or decoding fails
      }
    }
    return null; // No token or not authenticated
  }, [auth?.accessToken]);
  ...
  const handleLogout = () => {
    handleOpenLogout(); // Close the logout dialog
    ...
    <header className="flex w-full items-center gap-x-8 px-4 py-2">
      ...
      <div className="">
            ...
            <MenuHandler>
              <div className="flex cursor-pointer items-center justify-center gap-2 px-2 hover:text-blue-600">
                <Avatar
                  variant="circular"
                  alt="user_avatar"
                  className="cursor-pointer shadow-sm h-full object-contain"
                  style={{ maxWidth: "40px" }}
            ...
            <MenuList>
              ...
              <MenuItem className="flex items-center gap-2">
                ...
                <Typography
                  ...
                  onClick={handleOpenLogout} // Trigger dialog
                >
                  Đăng xuất
                </Typography>
              </MenuItem>
        ...
        <Dialog open={openLogout} handler={handleOpenLogout} size="sm">
          ...
          <DialogBody>
            Bạn xác nhận muốn đăng xuất khỏi hệ thống?
          </DialogBody>{" "}
          {/* Added question mark for better UX */}
          <DialogFooter>
            <Button
              variant="text"
              color="red"
